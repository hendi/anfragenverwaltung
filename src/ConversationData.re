...
let apiBaseUrl = if (isProd) {""} else {"http://localhost:8000"};
...
module Decode = {
  ...
  and many_conversations = json =>
    json |> Json.Decode.array(single_conversation)
  and many_messages = (json): array(message) =>
    json |> Json.Decode.array(single_message)
  and many_attachments = (json): array(attachment) =>
    json |> Json.Decode.array(single_attachment);
};
  ...
  let json =
    Json.Encode.(
      object_([
        (
          "conversation_ids",
          array(int, Array.map(c => c.id, conversations)),
        ),
...
let rateConversation =
    (conversation: conversation, rating: option(rating), callback) => {
...
let setReadStatusForConversation =
    (conversation: conversation, is_read: bool, callback) => {
  let data = Js.Dict.empty();
  Js.Dict.set(data, "is_read", Js.Json.boolean(is_read));
  changeConversation(conversation, data, callback);
};

let storeNotesForConversation =
    (conversation: conversation, notes: string, callback) => {
  let data = Js.Dict.empty();
  Js.Dict.set(data, "notes", Js.Json.string(notes));
  changeConversation(conversation, data, callback);
};

let trashConversation =
    (conversation: conversation, is_in_trash: bool, callback) => {
  let data = Js.Dict.empty();
  Js.Dict.set(data, is_in_trash ? "trash" : "untrash", Js.Json.string("x"));
  changeConversation(conversation, data, callback);
};

let ignoreConversation =
    (conversation: conversation, is_ignored: bool, callback) => {
  let data = Js.Dict.empty();

...
let component = ReasonReact.reducerComponent("MassReply");
...
let make = (~conversations, ~onMassReplySent, _children) => {
  let cbSent = self => {
    self.ReasonReact.send(ReplySent);
    1;
  };
  {
    ...component,
    ...
    reducer: (action, state) =>
        ...
        ReasonReact.UpdateWithSideEffects(
          ...
          (
            self => {
              ...
              onMassReplySent(
                conversations, self.state.message_text, attachments, _ =>
                cbSent(self)
              );
            }
          ),
      ...
      | ReplySent =>
        ReasonReact.Update({...state, sending: false, sent: true})
      },
    render: self =>
      <div className="MassReply">
        ...
        <p className="info">
          {
            textEl(
              {js|Hinweis: Die Empänger der Nachricht sehen nicht, dass es sich um eine Sammelantwort handelt.|js},
            )
          }
        </p>
        <div className="recipient-list">
          <div> <strong> {textEl({js|Empfänger:|js})} </strong> </div>
          {
            conversations
            |> Array.map((conversation: conversation) =>
                 <div
                   className="recipient-list-item"
                   key={"recipient_" ++ string_of_int(conversation.id)}>
